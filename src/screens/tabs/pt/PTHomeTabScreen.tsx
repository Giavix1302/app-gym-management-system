...
import { RootStackParamList, PTTabParamList } from '../../../navigation/types';
import { getUser } from '../../../utils/storage';
import { User, PTEvent } from '../../../types/api';
...
import { formatDateForAPI, isSameDayCheck, convertUTCToVietnam } from '../../../utils/dateTime';
import PTEventCard from '../../../components/schedule/PTEventCard';

type NavigationProp = CompositeNavigationProp<
  BottomTabNavigationProp<PTTabParamList, 'PTHomeTab'>,
...
export default function PTHomeTabScreen() {
  ...
  const [todayEvents, setTodayEvents] = useState<PTEvent[]>([]);
  ...
  const fetchTodayEvents = useCallback(async () => {
    ...
    try {
      ...
      const response = await trainerEventService.getTrainerEvents(user._id, {
        viewType: 'day',
      ...
      const filtered = eventsWithType.filter((event) => {
  ...
  const handleEventPress = () => {
    navigation.navigate('PTMySchedule');
  };
          ...
          {loadingEvents ? (
            ...
            <View className="items-center justify-center rounded-2xl bg-white p-8 shadow-sm">
              ...
              <Text className="text-center text-sm text-gray-500">
                Bạn chưa có buổi dạy hoặc lớp học nào hôm nay
              </Text>
            </View>
          ) : (
            <View className="rounded-2xl bg-white p-4 shadow-sm">
              ...
              <ScrollView
                ...
                {todayEvents.map((event, index) => (
                  <TouchableOpacity key={event._id || index} onPress={handleEventPress} activeOpacity={0.7}>
                    <PTEventCard event={event} />
                  </TouchableOpacity>
                ))}
        ...
        <View className="rounded-2xl border border-primary/20 bg-gradient-to-r from-primary/10 to-blue-50 p-4">
          <View className="flex-row items-start">
            ...
            <View className="flex-1">
              <Text className="mb-1 text-sm font-semibold text-gray-800">Mẹo huấn luyện</Text>
              <Text className="text-xs leading-5 text-gray-600">
                Hãy lên kế hoạch bài tập chi tiết và theo dõi tiến độ của học viên thường xuyên để
