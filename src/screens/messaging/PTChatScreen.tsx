// PTChatScreen - Chat screen for PT role
// Chatting with PT (Student)
...
type ChatRouteProp = RouteProp<RootStackParamList, 'PTChat'>;
type NavigationProp = NativeStackNavigationProp<RootStackParamList, 'PTChat'>;

export default function PTChatScreen() {
  const route = useRoute<ChatRouteProp>();
  const navigation = useNavigation<NavigationProp>();
  const { error: showError } = useNotification();

  const { conversationId, student } = route.params;
  ...
  // Determine current user role - PT should use 'pt' role for API calls
  const currentUserRole = 'pt';
  ...
  const handleSend = async () => {
    ...
    const tempMessage: Message & { isOptimistic?: boolean } = {
      ...
      senderId: user._id,
      senderType: 'trainer', // PT is the trainer
  ...
  const renderItem = ({ item }: { item: Message }) => {
    ...
    return (
      <MessageBubble
        ...
        isRead={item.isRead}
        otherUserAvatar={student.avatar}
  ...
  const avatarSource = getAvatarSource(student.avatar);
