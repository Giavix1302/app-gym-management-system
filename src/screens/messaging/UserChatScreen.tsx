// UserChatScreen - Chat screen for User role
// Chatting with PT (Trainer)
...
type ChatRouteProp = RouteProp<RootStackParamList, 'UserChat'>;
type NavigationProp = NativeStackNavigationProp<RootStackParamList, 'UserChat'>;
...
export default function UserChatScreen() {
  ...
  const { conversationId, trainer } = route.params;
  ...
  // Determine current user role - User should always use 'user' role for API calls
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (data: Message | Message[]) => {
      // ✅ FIX: Backend emits array, need to unwrap
      const message = Array.isArray(data) ? data[0] : data;

      console.log('[ChatScreen] ✅ NEW MESSAGE EVENT RECEIVED:', message);

      if (!message || message.conversationId !== conversationId) {
        return; // Ignore if invalid or not for current conversation
      }
    ...
    return () => {
      ...
      socketService.offUserStopTyping(handleUserTyping);
      socketService.offMessagesRead(handleMessagesRead);
      socketService.offJoinedConversation(handleJoinedConversation);
    };
  }, [user?._id, conversationId]); // Only re-run when userId or conversationId changes
  ...
  const handleSend = async () => {
    ...
    const tempMessage: Message & { isOptimistic?: boolean } = {
      ...
      senderId: user._id,
      senderType: 'user', // User is always the sender in UserChatScreen
  ...
  const renderItem = ({ item }: { item: Message }) => {
    ...
    return (
      <MessageBubble
        ...
        isRead={item.isRead}
        otherUserAvatar={trainer.avatar}
  ...
  const avatarSource = getAvatarSource(trainer.avatar);
  const initials = getInitials(trainer.fullName);
